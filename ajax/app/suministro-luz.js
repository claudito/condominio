...
function loadTabla(page){
  ...
  function factor_mensual(page){
    var parametros = {"action":"ajax","page":page};
    $("#loader").fadeIn('slow');
    $.ajax({
      url:'../template/modal/'+objeto+'/factor.php',
      data: parametros,
       beforeSend: function(objeto){
      $("#loader_factor").html("cargando..");
      },
      success:function(data){
        $("#tabla_factor").html(data).fadeIn('slow');
        $("#loader_factor").html("");
      }
    })
  }
